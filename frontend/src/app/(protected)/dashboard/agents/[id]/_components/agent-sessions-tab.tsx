/**
 * Sessions tab — displays sessions belonging to this agent.
 * Reuses existing session list patterns with animated cards.
 */
...
const KIND_CONFIG: Record<string, { variant: GradientVariant; label: string; gradient: string }> = {
    main: { variant: "success", label: "Main", gradient: "from-emerald-500/10 to-emerald-500/5" },
    group: { variant: "info", label: "Group", gradient: "from-blue-500/10 to-blue-500/5" },
    cron: { variant: "warning", label: "Cron", gradient: "from-amber-500/10 to-amber-500/5" },
    hook: { variant: "purple", label: "Hook", gradient: "from-purple-500/10 to-purple-500/5" },
    node: { variant: "primary", label: "Node", gradient: "from-primary/10 to-primary/5" },
    other: { variant: "secondary", label: "Other", gradient: "from-muted to-muted/50" },
}

export function AgentSessionsTab({ sessions }: AgentSessionsTabProps) {
    if (!sessions.length) {
        return (
            <EmptyState
                icon={Radio}
                title="No sessions"
                description="No sessions found for this agent."
            />
        )
    }
